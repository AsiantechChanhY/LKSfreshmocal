...
public class DisputeOrder_OpenTab extends HockeyFragment
{
	...
	private ArrayList<DisputeOrder_Pojo> itemList;
	...
	private DisputeOrderAdapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootview =inflater.inflate(R.layout.dispute_order_open_tab,container,false);
								...
								if(show_progress_status)
								{
									...
									boolean isInternetPresent = cd.isConnectingToInternet();
										...
										if (isInternetPresent) 
										{
											...
											{
												Loadmore_JsonRequest(Iconstant.dispute_order_open_url + userid+"&type=open"+"&pageId="+checkpagepos);
											}
										} 
										else 
										{
				...
				boolean isInternetPresent = cd.isConnectingToInternet();
						...
						if (isInternetPresent) 
						{
							if(session.isLoggedIn())
							{
								...
								JsonRequest(Iconstant.dispute_order_open_url + userid+"&type=open"+"&pageId=1");
							}
						} 
						else
						{
	...
	private void initialize(View rootview) 
	{
		...
		itemList = new ArrayList<DisputeOrder_Pojo>();
		cd = new ConnectionDetector(getActivity());
		isInternetPresent = cd.isConnectingToInternet();
		
		session = new SessionManager(getActivity());	
		
		listview=(ListView)rootview.findViewById(R.id.dispute_order_opentab_listview);
		loading=(RelativeLayout)rootview.findViewById(R.id.dispute_order_opentab_loading_layout);
		nointernet=(RelativeLayout)rootview.findViewById(R.id.dispute_order_opentab_nointernet_layout);
		mainlayout=(RelativeLayout)rootview.findViewById(R.id.dispute_order_opentab_main_layout);
		open_loading=(ImageView)rootview.findViewById(R.id.dispute_order_opentab_loading_gif);
		emptyText = (TextView)rootview.findViewById(R.id.dispute_order_opentab_list_empty);
		
		swipeRefreshLayout = (SwipeRefreshLayout)rootview.findViewById(R.id.dispute_order_opentab_swipe_refresh_layout);
	...
	{
		float_menu=(FloatingActionMenu)rootview.findViewById(R.id.dispute_order_open_floating_menu);
		 ...
		 jsonObjReq = new JsonObjectRequest(Method.GET,Url, null, new Response.Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) 
					{
						...
						try {
							...
							if (jarray.length() > 0) 
							{
								...
								for (int i = 0; i < jarray.length(); i++)
								{
									...
									DisputeOrder_Pojo items = new DisputeOrder_Pojo();
						...
						if(show_progress_status)
						{
							...
							adapter = new DisputeOrderAdapter(getActivity(),itemList);
