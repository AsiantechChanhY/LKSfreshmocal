...
import com.shopsy.adapter.ActivityTabAdapter;
import com.shopsy.app.Add_Item_1;

public class ActivityTab extends HockeyFragment {
	private Context context;
	private ActionBar actionBar;
	private SessionManager session;
	private String userid;
	private ArrayList<ActivityTab_Pojo> itemList;
	...
	private JsonObjectRequest loadmore_jsonObjReq;
	private String asyntask_name = "normal";
	private ImageView activity_loading;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootview = inflater
				.inflate(R.layout.activitytab, container, false);
		context = getActivity();
							...
							if (show_progress_status) {
								...
								if (isInternetPresent) {
									if (session.isLoggedIn()) {
										Loadmore_JsonRequest(Iconstant.activityTab_url
												+ userid
												+ "?pageId="
												+ checkpagepos);
									}
					...
					public void onRefresh() {
						...
						if (isInternetPresent) {
							if (session.isLoggedIn()) {
								...
								JsonRequest(Iconstant.activityTab_url + userid
										+ "?pageId=0");
							}
						} else {
	...
	{
		float_menu=(FloatingActionMenu)rootview.findViewById(R.id.activitytab_floating_menu);
        ...
        Orders_Fab.setOnClickListener(new OnClickListener() 
        {
			...
			public void onClick(View v) {
				...
				Intent intent=new Intent(getActivity(),OrdersPage.class);
	        	startActivity(intent);
	        	getActivity().overridePendingTransition(R.anim.enter, R.anim.exit);
	...
	private void initialize(View rootview) {
		itemList = new ArrayList<ActivityTab_Pojo>();
		cd = new ConnectionDetector(getActivity());
		isInternetPresent = cd.isConnectingToInternet();

		session = new SessionManager(getActivity());

		listview = (ListView) rootview.findViewById(R.id.activitytab_listview);
		loading = (RelativeLayout) rootview
				.findViewById(R.id.activitytab_loading_layout);
		nointernet = (RelativeLayout) rootview
				.findViewById(R.id.activitytab_nointernet_layout);
		mainlayout = (RelativeLayout) rootview
				.findViewById(R.id.activitytab_main_layout);
		activity_loading = (ImageView) rootview
				.findViewById(R.id.activitytab_loading_gif);
		emptyText = (TextView) rootview
				.findViewById(R.id.activitytab_list_empty);

		swipeRefreshLayout = (SwipeRefreshLayout) rootview
				.findViewById(R.id.activitytab_swipe_refresh_layout);
		...
		if (isInternetPresent) {
			if (session.isLoggedIn()) {
				nointernet.setVisibility(View.GONE);
				JsonRequest(Iconstant.activityTab_url + userid + "?pageId=0");
			}
		} else {
			mainlayout.setVisibility(View.GONE);
			nointernet.setVisibility(View.VISIBLE);
	...
	private void JsonRequest(String Url) {
		...
		System.out.println("--------------activity tab url-------------------"
				+ Url);
		...
		jsonObjReq = new JsonObjectRequest(Method.GET, Url, null,
				new Response.Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) {
						System.out
								.println("--------------activity tab response -----------"
										+ response);

						try {
							JSONArray jarray = response
									.getJSONArray("shopActivity");
							...
							if (jarray.length() > 0) {
								...
								for (int i = 0; i < jarray.length(); i++) {
									...
									ActivityTab_Pojo items = new ActivityTab_Pojo();
									items.setActivitytext(object
											.getString("activitytext"));
									items.setActivitydate(object
											.getString("activitydate"));
									items.setSortkey(object
											.getString("sortkey"));
									...
									items.setType(object.getString("type"));
						...
						// ------code to close navigation drawer on homePage---
						//NavigationDrawerPage.CloseDrawer();

						if (show_progress_status) {

							adapter = new ActivityTabAdapter(getActivity(),
									itemList);
	...
	private void Loadmore_JsonRequest(String Url) {
		...
		System.out
				.println("--------------Activity Tab loadmore url-------------------"
						+ Url);
		...
		loadmore_jsonObjReq = new JsonObjectRequest(Method.GET, Url, null,
				new Response.Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) {
						System.out
								.println("--------------Activity Tab loadmore response -----------"
										+ response);
