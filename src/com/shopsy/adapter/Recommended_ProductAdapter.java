...
public class Recommended_ProductAdapter extends BaseAdapter 
{
	...
	public Recommended_ProductAdapter(Context c,ArrayList<Recommended_Pojo> d) 
	{
		context=c;
	...
	public View getView(int position, View convertView, ViewGroup parent) 
	{
		...
		holder.tvName.setText(data.get(position).getFeature_product_productName().replace("&#8230;", "..."));
		holder.tvprice.setText(data.get(position).getFeature_product_Price());
		holder.tvstorename.setText(data.get(position).getFeature_product_storeName());
		ImageView image = holder.image;
		Picasso.with(context).load(String.valueOf(data.get(position).getFeature_product_Image())).placeholder(R.drawable.no_image_grey).memoryPolicy(MemoryPolicy.NO_CACHE).into(image);
