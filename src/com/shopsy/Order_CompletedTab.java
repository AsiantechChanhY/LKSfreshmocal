...
import com.shopsy.Pojo.OrderCompleted_Pojo;
import com.shopsy.Utils.AppController;
import com.shopsy.Utils.ConnectionDetector;
import com.shopsy.Utils.SessionManager;
import com.shopsy.adapter.OrderCompleted_Adapter;

public class Order_CompletedTab extends HockeyFragment
{
	...
	private ArrayList<OrderCompleted_Pojo> itemList;
	...
	private ImageView completed_loading;
	
	private OrderCompleted_Adapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootview =inflater.inflate(R.layout.order_completedtab,container,false);
								...
								if(show_progress_status)
								{
									...
									boolean isInternetPresent = cd.isConnectingToInternet();
										...
										if (isInternetPresent) 
										{
											...
											{
												Loadmore_JsonRequest(Iconstant.order_completed_url + userid+"&type=completed"+"&pageId="+checkpagepos);
											}
										} 
										else 
										{
				...
				boolean isInternetPresent = cd.isConnectingToInternet();
						...
						if (isInternetPresent) 
						{
							if(session.isLoggedIn())
							{
								...
								JsonRequest(Iconstant.order_completed_url + userid+"&type=completed"+"&pageId=1");
							}
						} 
						else
						{
	...
	private void initialize(View rootview) 
	{
		...
		itemList = new ArrayList<OrderCompleted_Pojo>();
		cd = new ConnectionDetector(getActivity());
		isInternetPresent = cd.isConnectingToInternet();
		
		session = new SessionManager(getActivity());	
		
		listview=(ListView)rootview.findViewById(R.id.order_completedtab_listview);
		loading=(RelativeLayout)rootview.findViewById(R.id.order_completedtab_loading_layout);
		nointernet=(RelativeLayout)rootview.findViewById(R.id.order_completedtab_nointernet_layout);
		mainlayout=(RelativeLayout)rootview.findViewById(R.id.order_completedtab_main_layout);
		completed_loading=(ImageView)rootview.findViewById(R.id.order_completedtab_loading_gif);
		emptyText = (TextView)rootview.findViewById(R.id.order_completedtab_list_empty);
		
		swipeRefreshLayout = (SwipeRefreshLayout)rootview.findViewById(R.id.order_completedtab_swipe_refresh_layout);
	...
	private void JsonRequest(String Url) 
	{
		...
		System.out.println("--------------ordercompleted tab url-------------------"+Url);
		
		 jsonObjReq = new JsonObjectRequest(Method.GET,Url, null, new Response.Listener<JSONObject>() {
					@Override
					public void onResponse(JSONObject response) 
					{
						System.out.println("--------------ordercompleted tab response -----------"+response);
						...
						try {
							...
							if (jarray.length() > 0) 
							{
								...
								for (int i = 0; i < jarray.length(); i++)
								{
									...
									OrderCompleted_Pojo items = new OrderCompleted_Pojo();
						...
						if(show_progress_status)
						{
							...
							adapter = new OrderCompleted_Adapter(getActivity(),itemList);
...
// -------------------------code for JSON Request LoadMore----------------------------------
			...
			private void Loadmore_JsonRequest(String Url) 
			{
				...
				System.out.println("--------------ordercompleted loadmore url-------------------"+Url);
				
				loadmore_jsonObjReq = new JsonObjectRequest(Method.GET,Url, null, new Response.Listener<JSONObject>() {
							@Override
							public void onResponse(JSONObject response) 
							{
								System.out.println("--------------ordercompleted loadmore response -----------"+response);
